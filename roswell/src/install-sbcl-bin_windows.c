...
int sbcl_bin_install(struct install_options* param) {
  ...
  char* home=configdir();
  char *base,*str,*str2,*str3,*str4;
  char* version_num= q(version);
  int ret;
  base=cat(home,"src\\sbcl-",version,"-",arch,"-windows\\PFiles\\Steel Bank Common Lisp\\",version_num,NULL);
  if(!directory_exist_p(base)) {
    s(base);
    base=cat(home,"src\\sbcl-",version,"-",arch,"-windows\\PFiles\\Steel Bank Common Lisp",NULL);
  }
  str2=cat(base,"\\sbcl.exe",NULL);
